...
import { computed, onMounted, type StyleValue, watch } from 'vue'
...
export const useTooltip = ({
  ...
  const middleware = [offset(_offset), flip(), shift({ padding })]
  if (arrowElement) {
    middleware.push(arrow({ element: arrowElement, padding: 6 }))
  }
  ...
  if (options.handleDialogModals) {
    watch(show, (value) => {
      ...
      if (!dialogEl) {
        if (tooltipElement.value.parentElement !== initialParent) {
          initialParent.appendChild(tooltipElement.value)
        }
        return
      }
      const isModal =
        globalThis.getComputedStyle(dialogEl, '::backdrop').display !== 'none'
      if (isModal) {
